        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection connection = implementConnection()) {
                ...
                PreparedStatement fetch = connection.prepareStatement(
                        "SELECT `uuid`,`name`,`UnlockedPets`,`PetName`,`NeedsRespawn`,`SavedPets` " +
                                "FROM `" + SQLData.TABLE_PREFIX + "_players` WHERE `uuid`=?;"
                );
                PreparedStatement delete = connection.prepareStatement(
                        "DELETE FROM `" + SQLData.TABLE_PREFIX + "_players` WHERE `uuid`=?;"
                );
                PreparedStatement insert = connection.prepareStatement(
                        "INSERT INTO `" + SQLData.TABLE_PREFIX + "_players` " +
                                "(`uuid`, `name`, `UnlockedPets`, `PetName`, `NeedsRespawn`, `SavedPets`) " +
                                "VALUES (?, ?, ?, ?, ?, ?);"
                );
